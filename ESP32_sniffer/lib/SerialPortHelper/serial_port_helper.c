...
void serial_port_init(const serial_port_num serial_num)
{
    ...
    if (serial_num == SERIAL_1)
	{
		ESP_ERROR_CHECK(uart_set_line_inverse(serial_num, UART_SIGNAL_TXD_INV));
	}

	if (serial_num == SERIAL_2)
	{
		ESP_ERROR_CHECK(uart_set_line_inverse(serial_num, UART_SIGNAL_RXD_INV));
	}

	ESP_ERROR_CHECK(uart_driver_install(serial_num, 1024, 1024, 0, NULL, 0));
...
void serial_port_read()
{
	while (1)
	{
		...
		if (serial_port_configs[serial_num].init)
		{
			...
			sd.length = uart_read_bytes(serial_num, sd.buffer, SERIAL_PORT_RD_BYTES_NUM, 1 / portTICK_PERIOD_MS);
